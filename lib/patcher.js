...
function getDebugInfo(filePath, options = {}) {
  ...
  try {
    ...
    // Count marker words found, keep only a sample for display
    for (const word of MARKER_WORDS) {
      if (str.includes(`"${word}"`)) {
        debug.markerWordsFound.push(word);
      }
    }
    debug.markerWordsCount = debug.markerWordsFound.length;
    debug.markerWordsFound = debug.markerWordsFound.slice(0, 6);
